...
**安装成功不等于自动回传已打通。** 插件加载、命令可用，只能说明基础追踪层生效；最终异步回传还依赖宿主实际暴露的 `agent:end`、`message:sent`、`api.sendMessage()` 契约。
...
- **`message:sent` / `message_sent` 事件**：确认宿主发送层已接受或确认发送，不保证 Telegram 终端已实际收到
...
- **默认值 `0`**：禁用文本长度检测
- **默认值 `0` 时，普通 Telegram 文本消息不会自动进入异步链路**
...
- 不同 OpenClaw 版本的 hook shape 可能不同
...
| `contracts=[...]` | 宿主事件契约观测结果：`inbound`、`agent`、`outbound` 分别表示入站 normalize、完成关联、发送确认关联的强弱 |
| `classification=...` | 当前异步识别模式：`explicit_only` / `threshold_fallback` / `hybrid` |
| `recent=<n>` | 最近被追踪任务数量 |
| `latest=<task:state>` | 最近任务摘要 |
...
若仍使用默认配置 `asyncTextLengthThreshold: 0`，普通 Telegram 文本不会自动进入异步链路；验证时请使用 `asyncReturn: true`、异步标签或显式业务标记。
...
| `contracts=[outbound:weak,...]` | `message:sent` 缺少 `taskId` | 补齐 taskId，或至少保证 chatId 可用于弱关联 |
...
通过 OpenClaw 扩展目录加载时，优先使用插件命令：

```text
/async-return ...
```

只有在 npm / global 安装后，且运行环境显式暴露了 bin，才可以直接使用独立 CLI。
...
5. `sent_confirmed` → 告知宿主已确认发送，可重发
6. `failed` → 告知执行失败，可重跑
...
4. **`agent:end` 事件字段可能与插件预期不匹配**
   - 插件会先 normalize 原始事件，再尝试按 `taskId`、`sessionId`、`chatId`、`sessionKey` 关联
   - 若全部缺失，插件会记录 contract mismatch 日志但无法关联到已追踪任务

5. **按文本长度自动识别长任务可能误判**
   - 默认已禁用（`asyncTextLengthThreshold: 0`）
   - `asyncTextLengthThreshold: 0` 时，普通 Telegram 文本不会自动进入异步链路
